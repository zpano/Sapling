/**
 * Sapling 配置管理模块（TypeScript）
 */

export const CEFR_LEVELS = ['A1', 'A2', 'B1', 'B2', 'C1', 'C2'] as const;
export type CefrLevel = (typeof CEFR_LEVELS)[number];

export type PronunciationProvider = 'wiktionary' | 'youdao' | 'google' | string;
export type OutputFormat = 'standard' | 'toon' | string;
...
export type IntensityConfig = Record<IntensityKey, IntensitySetting>;
...
export interface SaplingConfig {
  ...
  youdaoPronunciationType: number;
  enabled: boolean;
...
// 支持的语言
export const SUPPORTED_LANGUAGES: { native: LanguageOption[]; target: LanguageOption[] } = {
  native: [
 ...
 * 规范化用户配置的缓存容量上限
 */
export function normalizeCacheMaxSize(value: unknown, fallback = CACHE_CONFIG.maxSize): number {
...
function normalizeIntInRange(value: unknown, fallback: number, { min, max }: { min: number; max: number }): number {
  const parsed = Number.parseInt(String(value), 10);
  if (!Number.isFinite(parsed)) return fallback;
  return Math.min(max, Math.max(min, parsed));
}

export function normalizeConcurrencyLimit(value: unknown, fallback = DEFAULT_CONFIG.concurrencyLimit): number {
  return normalizeIntInRange(value, fallback, ADVANCED_LIMITS.concurrencyLimit);
}

export function normalizeMaxBatchSize(value: unknown, fallback = DEFAULT_CONFIG.maxBatchSize): number {
  return normalizeIntInRange(value, fallback, ADVANCED_LIMITS.maxBatchSize);
}

export function normalizeMaxTokens(value: unknown, fallback = DEFAULT_CONFIG.maxTokens): number {
 ...
 * 判断词汇难度是否符合用户设置
 */
export function isDifficultyCompatible(wordDifficulty: CefrLevel | string, userDifficulty: CefrLevel | string): boolean {
  const wordIdx = CEFR_LEVELS.indexOf(wordDifficulty as CefrLevel);
  const userIdx = CEFR_LEVELS.indexOf(userDifficulty as CefrLevel);
