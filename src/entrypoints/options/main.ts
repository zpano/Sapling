...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  function bindEvents() {
    ...
    elements.testConnectionBtn.addEventListener('click', async () => {
      ...
      } satisfies TestApiRequest, (response: TestApiResponse) => {
    ...
    elements.clearLearnedBtn.addEventListener('click', async () => {
      ...
      if (!ok) return;
      chrome.runtime.sendMessage({ action: 'clearLearnedWords' } satisfies ClearLearnedWordsRequest, (_response: SimpleSuccessResponse) => {
        loadSettings();
        debouncedSave(200);
      });
    });
    ...
    elements.clearMemorizeBtn.addEventListener('click', async () => {
      ...
      if (!ok) return;
      chrome.runtime.sendMessage({ action: 'clearMemorizeList' } satisfies ClearMemorizeListRequest, (_response: SimpleSuccessResponse) => {
        loadSettings();
        debouncedSave(200);
      });
    });
    ...
    elements.clearCacheBtn.addEventListener('click', async () => {
      ...
      if (!ok) return;
      chrome.runtime.sendMessage({ action: 'clearCache' } satisfies ClearCacheRequest, (_response: SimpleSuccessResponse) => {
        loadSettings();
        debouncedSave(200);
      });
    });
