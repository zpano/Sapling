...
import type { WordCacheItem } from '~/types/storage';

type CacheValue = Omit<WordCacheItem, 'key'> & { timestamp?: number };
...
class CacheService {
   ...
   * @returns {string}
   */
  generateKey(word: string, sourceLang: string, targetLang: string) {
   ...
   * @returns {object|null}
   */
  get(word: string, sourceLang: string, targetLang: string): CacheValue | null {
