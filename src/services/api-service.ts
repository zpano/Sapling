 ...
 */
class ApiService {
  private config: SaplingConfig | null;
  ...
  _runLimited(task: () => Promise<unknown>) {
   ...
   */
  setConfig(config: SaplingConfig) {
    this.config = config;
  ...
  parseBatchApiResponse(content) {
    ...
    // 循环解包结果（处理多层嵌套如 {results: {results: [...]}}）
    // 设置最大深度防止死循环
    let depth = 0;
    while (depth < 3) {
      if (Array.isArray(parsed)) {
        ...
        // 如果数组元素看起来像是在描述单词 (有 original 或 word 字段)
        if (parsed.length > 0 && (parsed[0].original || parsed[0].word)) {
        ...
        // 这是一个未知的数组结构，可能是嵌套的，尝试取第一项如果它是数组
        if (Array.isArray(parsed[0])) {
          parsed = parsed[0]; // 解包一层数组
          depth++;
          continue;
        }
