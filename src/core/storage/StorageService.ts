...
import { StorageNamespace } from '~/core/storage/StorageNamespace';
import type { GetResult, Keys, LearnedWord, LocalStorageData, MemorizeItem, RemoteStorageData } from '~/types/storage';

/**
 * 存储服务类
 */
class StorageService {
  remote: StorageNamespace<RemoteStorageData>;
  local: StorageNamespace<LocalStorageData>;

  constructor() {
    // 创建存储命名空间
    const remoteAdapter = new ChromeStorageAdapter<RemoteStorageData>('sync');
    const localAdapter = new ChromeStorageAdapter<LocalStorageData>('local');
   ...
   */
  async get<K extends Keys<RemoteStorageData>>(keys: K = null as K): Promise<GetResult<RemoteStorageData, K>> {
   ...
   */
  async set(items: Partial<RemoteStorageData>): Promise<void> {
   ...
   * @returns {Promise<object>}
   */
  async getLocal<K extends Keys<LocalStorageData>>(keys: K = null as K): Promise<GetResult<LocalStorageData, K>> {
   ...
   * @returns {Promise<void>}
   */
  async setLocal(items: Partial<LocalStorageData>): Promise<void> {
   ...
   * @returns {Promise<void>}
   */
  async removeLocal(keys: keyof LocalStorageData | readonly (keyof LocalStorageData)[]): Promise<void> {
   ...
   * @returns {Promise<object>}
   */
  async updateStats(stats: { newWords?: number; cacheHits?: number; cacheMisses?: number }) {
   ...
   * @returns {Promise<void>}
   */
  async addToWhitelist(word: Pick<LearnedWord, 'original' | 'word'>) {
   ...
   * @returns {Promise<void>}
   */
  async addToMemorizeList(word: MemorizeItem['word']) {
