...
import type { CefrLevel, OutputFormat } from '~/types/config';
...
export function buildBatchVocabularySelectionPrompt({
  ...
  paragraphs: Array<{ index: number; text: string; sourceLang: string; targetLang: string }>;
  nativeLanguage: string;
  learningLanguage: string;
  aiTargetCount: number;
  aiMaxCount: number;
